...
import { createListingSubmission, markCheckoutFailed, attachCheckoutSession } from "@/lib/listing-submission-store";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (value(form, "_honey", 200)) return NextResponse.json({ ok: true, checkoutUrl: "/" });

    const certifications = ["authority_certification", "accuracy_certification", "marketplace_acknowledgment", "fee_agreement"];
    if (!certifications.every((key) => accepted(form, key))) {
      return NextResponse.json({ error: "Please accept all broker certifications before continuing." }, { status: 400 });
    }
    ...
    if (!brokerageName || !contactPreference) {
      return NextResponse.json({ error: "Please complete the required brokerage and inquiry-routing fields." }, { status: 400 });
    }

    const document = form.get("supporting_document");
    const storedDocument = document instanceof File && document.size
    ...
    const checkout = await createListingCheckoutSession(submission, request.url, "/brokers/list-your-license?payment=cancelled");
    await attachCheckoutSession(submission.id, checkout.id);

    return NextResponse.json({ ok: true, submissionRef: submission.submissionRef, checkoutUrl: checkout.url });
  } catch (error) {
    const message = error instanceof Error ? error.message : "The broker listing could not be submitted.";
    if (submissionId) {
      try { await markCheckoutFailed(submissionId, message); } catch (markError) { console.error("Could not mark broker checkout as failed", markError); }
    }
