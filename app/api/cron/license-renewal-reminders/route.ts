...
export async function GET(request: Request) {
  ...
  for (const reminder of reminders) {
    try {
      ...
      await updateLicenseReminder(reminder.id, { status: "sent", sent_at: new Date().toISOString(), last_error: null }, reminder.objectName);
      sent += 1;
    } catch (error) {
      failed += 1;
      await updateLicenseReminder(reminder.id, {
        status: "failed",
        last_error: error instanceof Error ? error.message.slice(0, 1500) : "Email delivery failed",
      }, reminder.objectName);
    }
  }
  return Response.json({ processed: reminders.length, sent, failed });
}
