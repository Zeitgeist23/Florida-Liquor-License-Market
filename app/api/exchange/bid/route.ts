...
export async function POST(request: Request) {
  try {
    ...
    await sendFllmEmail({
      ...
      text: `${buyerName} submitted a ${money(price)} bid for ${listingRef}. Seller ask: ${money(ask)}. ${matched ? `The bid meets or exceeds the ask. A non-binding price match has been recorded at ${money(ask)}. Transaction room: ${roomUrl}` : `Review, accept, or counter securely: ${sellerReviewUrl}`}`,
      html: shell(`<h1 style="font-size:24px">${matched ? "Price Match Reached" : "New Buyer Bid"}</h1><p><strong>Listing:</strong> ${listingRef}<br><strong>Seller Ask:</strong> ${money(ask)}<br><strong>Buyer Bid:</strong> ${money(price)}<br><strong>Buyer:</strong> ${buyerName}<br><strong>Email:</strong> ${buyerEmail}<br><strong>Phone:</strong> ${buyerPhone}</p>${matched ? `<p><strong>A non-binding price match has been recorded at ${money(ask)}.</strong></p><p><a href="${roomUrl}">Open FLLM Transaction Room →</a></p>` : `<p><a href="${sellerReviewUrl}">Review / Accept / Counter Bid →</a></p>`}<p style="font-size:12px;color:#5b6670">A price match is not a binding purchase agreement. Closing, contingencies, regulatory approval, due diligence and other material terms remain to be agreed.</p>`),
    });
    ...
    await sendFllmEmail({
      ...
      text: matched
        ? `Your ${money(price)} bid meets the seller's ${money(ask)} ask. FLLM recorded a non-binding price match at ${money(ask)}. Transaction room: ${roomUrl}`
        : `Your ${money(price)} bid for ${listingRef} has been recorded. Seller ask: ${money(ask)}. Status link: ${buyerStatusUrl}`,
      html: shell(`<h1 style="font-size:24px">${matched ? "Price Match Reached" : "Bid Recorded"}</h1><p><strong>Listing:</strong> ${listingRef}<br><strong>Seller Ask:</strong> ${money(ask)}<br><strong>Your Bid:</strong> ${money(price)}</p>${matched ? `<p><strong>FLLM recorded a non-binding price match at ${money(ask)}.</strong></p><p><a href="${roomUrl}">Open FLLM Transaction Room →</a></p>` : `<p>The seller can now accept or counter your bid.</p><p><a href="${buyerStatusUrl}">View Bid Status →</a></p>`}<p style="font-size:12px;color:#5b6670">A price match or accepted bid is not a binding purchase agreement until final transaction terms are separately accepted by the parties.</p>`),
    });
