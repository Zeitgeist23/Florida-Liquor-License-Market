...
const FINANCING_PAGE_STYLES = `<style id="financing-logo-match-investment-v1">
  ...
  .fllm-financing-calculator-link {
    display:grid;
    ...
    margin:18px 0;
    padding:17px 18px;
    border:1px solid rgba(124,239,255,.35);
    border-radius:10px;
    background:linear-gradient(145deg,#09222b,#03151d);
    box-shadow:0 12px 28px rgba(0,0,0,.18),0 0 28px rgba(70,210,229,.08);
  }
  .fllm-financing-calculator-link span {
    display:block;
    margin-bottom:4px;
    color:#7cefff;
    font-size:10px;
  ...
  .fllm-financing-calculator-link h2 {
    margin:0 0 5px;
    color:#fff;
    font-size:22px;
  }
  .fllm-financing-calculator-link p {
    margin:0;
    color:#c9d8de;
    font-size:12px;
    line-height:1.55;
  }
  .fllm-financing-calculator-link a {
    display:inline-flex;
    align-items:center;
    min-height:42px;
    padding:0 15px;
    ...
    border-radius:6px;
    color:#062027;
    background:linear-gradient(180deg,#8af4ff,#49d5e5);
    font-size:11px;
    font-weight:900;
    text-decoration:none;
    text-transform:uppercase;
...
function optimizeFinancingHtml(input: string): string {
  ...
  if (!html.includes("fllm-financing-appraisal-card")) {
    html = html.replace('<div class="seller-trust">', `${APPRAISAL_CARD}${CALCULATOR_LINK}<div class="seller-trust">`);
  }
  html = html.replace(
