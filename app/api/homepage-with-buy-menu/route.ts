...
.primary-nav .live-nav-dropdown:hover .live-nav-menu,.primary-nav .live-nav-dropdown:focus-within .live-nav-menu{display:grid;gap:4px}
...
@media(max-width:820px){.primary-nav .live-nav-dropdown{width:100%;justify-content:center}.primary-nav .live-nav-trigger{width:100%;justify-content:center;padding:12px}.primary-nav .live-nav-menu,.primary-nav .live-sell-menu{top:100%;width:min(330px,calc(100vw - 24px))}}
</style>`;
...
(function(){
  function makeMenu(type){
    ...
    if(type==='buy'){
      wrap.className='live-nav-dropdown live-buy-dropdown';
      ...
      wrap.className='live-nav-dropdown live-sell-dropdown';
      wrap.setAttribute('data-live-sell-dropdown','true');
  ...
  function install(){
    ...
    if(!nav)return;
    if(!nav.querySelector('.live-buy-dropdown')){
      var buy=findPlainLink(nav,'buy','/listings');
      if(buy)buy.replaceWith(makeMenu('buy'));
    }
    if(!nav.querySelector('.live-sell-dropdown')){
      var sell=findPlainLink(nav,'sell','/sell-your-license');
      if(sell)sell.replaceWith(makeMenu('sell'));
    }
  }
...
<script src="/assets/finance-menu-options.js?v=1" defer></script>
