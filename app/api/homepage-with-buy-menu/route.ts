...
.primary-nav .live-resources-menu{left:auto;right:-18px;width:min(930px,calc(100vw - 32px));transform:none;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px 10px;padding:12px}
.primary-nav .live-resources-menu::before{content:"";position:absolute;top:-7px;right:46px;width:12px;height:12px;transform:rotate(45deg);border-left:1px solid #f6a700;border-top:1px solid #f6a700;background:#061728}
...
@media(max-width:980px){.primary-nav .live-resources-menu{width:min(720px,calc(100vw - 24px));grid-template-columns:repeat(2,minmax(0,1fr));right:-8px}}
@media(max-width:820px){.primary-nav .live-nav-dropdown{width:100%;justify-content:center;padding-bottom:0;margin-bottom:0}.primary-nav .live-nav-trigger{width:100%;justify-content:center;padding:12px}.primary-nav .live-nav-menu,.primary-nav .live-sell-menu,.primary-nav .live-license-types-menu,.primary-nav .live-resources-menu{top:100%;right:auto;left:50%;transform:translateX(-50%);width:min(360px,calc(100vw - 24px));grid-template-columns:1fr;max-height:70vh;overflow:auto}.primary-nav .live-resources-menu::before{right:auto;left:50%;transform:translateX(-50%) rotate(45deg)}.primary-nav .live-resources-menu .live-resource-transaction{grid-column:auto;display:block!important}.primary-nav .live-resource-transaction small,.primary-nav .live-resource-transaction em{display:block;margin-top:5px;text-align:left}}
</style>`;
...
(function(){
  ...
  function makeMenu(type){
    ...
    if(type==='buy'){
      ...
      wrap.innerHTML=${JSON.stringify(resourcesMenuMarkup.replace(' data-live-resources-dropdown="true"','').replace(/^<div class="live-nav-dropdown live-resources-dropdown">|<\/div>$/g,''))};
    }
  ...
  function install(){
    ...
    if(!nav)return;
    document.querySelectorAll('.resources-header-menu').forEach(function(el){el.remove();});
...
function stripLegacyResourcesScripts(html: string) {
  return html
...
function injectServerMenus(html: string) {
  let updated = stripLegacyResourcesScripts(html);
