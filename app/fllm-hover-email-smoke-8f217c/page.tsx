...
const TEST_TOKEN = "fllm-hover-smoke-8f217cb54b4e";
...
export default async function Page({
  ...
  try {
    ...
    const result = await sendFllmEmail({
      to: TEST_RECIPIENT,
      subject: `[FLLM HOVER TEST] ${built.subject}`,
      text: built.text,
      html: built.html,
    });
    ...
    return <main><h1>FLLM hover email test</h1><pre>{error instanceof Error ? error.message : String(error)}</pre></main>;
  }
}
