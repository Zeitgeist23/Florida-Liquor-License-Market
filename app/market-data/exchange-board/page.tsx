...
export const metadata: Metadata = {
  ...
  description:
    "View active Florida liquor license asking prices by county and license type on the FLLM Exchange Board, including current inventory, featured listings and market data.",
  ...
  openGraph: {
    ...
    title: "FLLM Exchange Board | Florida Liquor License Asking Prices",
    description:
      "Adding transparency to the Florida liquor license market with active asking prices, county market data and current FLLM inventory.",
...
export default async function ExchangeBoardPage() {
  ...
  const featuredTicker = [...featured, ...listings.filter((listing) => !featured.includes(listing))].slice(0, 12);
  ...
  const structuredData = {
    ...
    url: canonicalUrl,
    description:
      "Active Florida liquor license asking prices, county market data and featured FLLM marketplace inventory.",
    ...
    <main className={styles.page}>
          ...
          <div className={`${styles.tickerTrack} ${styles.tickerTrackSlow}`}>
            {[0, 1].map((cycle) => (
              <div className={styles.themeSequence} key={cycle}>
                ...
                {featuredTicker.map((listing, index) => (
      ...
      <section className={styles.marketGrid}>
        ...
        <article className={`${styles.panel} ${styles.exchangeBoard}`}>
          ...
          <div className={styles.boardTable}>
            <div className={styles.boardHeader}><span>County</span><span>License Type</span><span>Asking Price</span><span>Status</span></div>
        ...
        <aside className={styles.sideColumn}>
          <article className={styles.panel}>
            <div className={styles.panelTitle}><span>MARKET SNAPSHOT</span><small>FLORIDA LIQUOR LICENSE MARKET</small></div>
          ...
          <article className={styles.panel}>
            <div className={styles.panelTitle}><span>NEWEST TO MARKET</span></div>
            <div className={styles.newList}>
              {newest.map((listing) => (
                <Link href={listingPageHref(listing)} key={listing.sourceRef}><b>NEW</b><span>{shortCounty(listing.county)}</span><i>{shortType(listing.type)}</i><strong>{money(listing.price, listing.priceLabel)}</strong></Link>
              ))}
            </div>
          </article>
        </aside>
      ...
      {featuredListing && (
        <section className={styles.featuredBar}>
          <div><span>FEATURED MARKET LISTING</span><strong>{featuredListing.county} · {shortType(featuredListing.type)} · {money(featuredListing.price, featuredListing.priceLabel)} ASK</strong></div>
      ...
      <section className={styles.transparencySection}>
        ...
        <div className={styles.transparencyCards}>
          ...
          <article><strong>More Transparency</strong><span>A clearer statewide view without pretending asking prices are completed transactions.</span></article>
        </div>
      </section>
