...
export default async function ExchangeBoardPage() {
  ...
  const tickerA = listings.slice(0, 9);
  const tickerB = listings.slice(9, 18).length ? listings.slice(9, 18) : listings.slice(0, 9);
  ...
  return (
    <main className="exchange-page">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .exchange-header{position:relative;z-index:30;background:#020d18;border-bottom:1px solid rgba(246,167,0,.55)}
        .approved-hero{width:100%;background:#020b14;border-bottom:1px solid rgba(36,177,239,.34);overflow:hidden}
        .approved-hero img{display:block;width:100%;height:auto;max-height:570px;object-fit:cover;object-position:center}
        ...
        .ticker-line:nth-child(2) .ticker-track{animation-duration:62s;animation-direction:reverse}
        ...
        .market-panel{min-width:0;border:1px solid #176a90;background:linear-gradient(180deg,#071f34,#041321);box-shadow:inset 0 0 30px rgba(0,132,196,.06)}
        ...
        .heatmap-legend{display:grid;gap:6px;padding:0 14px 14px;color:#d8e4eb;font-size:10px}.heatmap-legend span{display:flex;align-items:center;gap:8px}.heatmap-legend i{display:block;width:12px;height:12px;border-radius:3px;background:#f5c54f}.heatmap-legend span:nth-child(2) i{background:#1e96df}.heatmap-legend span:nth-child(3) i{background:#083250}
        .gold-button{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:0 19px;border:1px solid #f6c145;border-radius:4px;background:linear-gradient(180deg,#ffd764,#eba315);box-shadow:0 0 14px rgba(246,181,31,.13);color:#06111b;text-decoration:none;font-size:10px;font-weight:950;text-transform:uppercase}
        ...
        @media(max-width:1000px){.market-grid{grid-template-columns:1fr 1.3fr}.market-grid>aside{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr;gap:12px}.action-grid{grid-template-columns:1fr 1fr}.approved-hero img{max-height:none}}
        @media(max-width:700px){.exchange-content{width:min(100% - 16px,680px)}.market-grid{grid-template-columns:1fr}.market-grid>aside{grid-column:auto;display:block}.newest-box{margin-top:10px}.board-head,.board-row{grid-template-columns:1fr .72fr 1fr}.board-head span:last-child,.board-row em{display:none}.news-bar{grid-template-columns:1fr;align-items:start}.news-bar span{text-align:left}.action-grid{grid-template-columns:1fr}.transparency-values{grid-template-columns:1fr}.approved-hero{overflow-x:auto}.approved-hero img{min-width:760px}}
      ` }} />
      ...
      <section className="approved-hero" aria-label="FLLM Exchange trading floor">
        <img
          src="/assets/fllm-exchange-board-hero.svg"
          alt="FLLM Exchange trading floor with Florida liquor license marketplace screens and Florida Liquor Licenses Trade Here banner"
        />
      </section>
      ...
      <section className="ticker-shell" aria-label="Active Florida liquor license asking prices">
        <div className="ticker-line">
          <div className="ticker-track">
            {[...tickerA, ...tickerA].map((listing, index) => (
              <Link className="ticker-item" href={listingPageHref(listing)} key={`a-${listing.sourceRef}-${index}`}>
                <b>{shortCounty(listing.county)}</b><span>{shortType(listing.type)}</span><strong>{money(listing.price, listing.priceLabel)}</strong><em>▲</em>
              </Link>
            ))}
          </div>
        </div>
        <div className="ticker-line">
          <div className="ticker-track">
            {[...tickerB, ...tickerB].map((listing, index) => (
      ...
      <div className="exchange-content">
        <section className="market-grid">
          <article className="market-panel">
            ...
            <div className="heatmap-legend">
              <span><i />Multiple Listings</span>
              <span><i />Active Listing</span>
              <span><i />No Active Listing</span>
            </div>
          ...
          <aside>
            ...
            <article className="market-panel newest-box">
              ...
              <div className="newest-list">
                {newest.map((listing) => (
                  <Link href={listingPageHref(listing)} key={listing.sourceRef}>
                    <b>NEW</b><span>{shortCounty(listing.county)} County</span><i>{shortType(listing.type)}</i><strong>{money(listing.price, listing.priceLabel)}</strong>
                  </Link>
                ))}
              </div>
        ...
        <section className="action-grid">
          <article className="action-card"><div className="action-icon">🛒</div><h2>FOR BUYERS</h2><p>Find available liquor licenses by county and license type.</p><Link className="gold-button" href="/listings">Browse Licenses</Link></article>
          <article className="action-card"><div className="action-icon">🏷</div><h2>FOR SELLERS</h2><p>List your Florida liquor license and reach qualified buyers.</p><Link className="gold-button" href="/sell-your-license">List Your License</Link></article>
          <article className="action-card"><div className="action-icon">👥</div><h2>FOR BROKERS</h2><p>Market your client inventory while remaining the listing representative.</p><Link className="gold-button" href="/brokers/list-your-license">Broker Options</Link></article>
          <article className="action-card"><div className="action-icon">▥</div><h2>MARKET DATA</h2><p>Explore pricing, trends and county market information.</p><Link className="gold-button" href="/#market-data">View Market Data</Link></article>
        </section>
      ...
      <section className="seo-copy">
        ...
        <p>Florida quota liquor license sales are fragmented across counties and are frequently marketed through private owners, brokers, attorneys and industry relationships. FLLM organizes active listings, asking prices, county market information, license types and transaction resources into one statewide marketplace.</p>
      </section>
    </main>
  );
}
