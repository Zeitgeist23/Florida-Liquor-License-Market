...
export const metadata: Metadata = {
  ...
  description: "Active Florida liquor license asking prices, county price discovery and current marketplace inventory.",
...
const HERO_URL = "https://raw.githubusercontent.com/Zeitgeist23/Florida-Liquor-License-Market/main/public/assets/fllm-exchange-board-header-live.png";
...
export default async function ExchangeBoardPage() {
  ...
  const tickerTwo = listings.slice(10, 20).length ? listings.slice(10, 20) : listings.slice(0, 10);
  ...
  return (
    <main className="exchange-page">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .panel{min-width:0;border:1px solid #176a90;background:linear-gradient(180deg,#071f34,#041321)}
        ...
        .board-head,.board-row{display:grid;grid-template-columns:1.12fr .82fr 1fr .76fr;gap:10px;align-items:center;padding:10px;border-bottom:1px solid rgba(255,255,255,.09)}
      ...
      <section className="hero-shell" aria-label="FLLM Exchange trading floor">
        <img src={HERO_URL} alt="FLLM Exchange trading floor with brokers, market displays and Florida Liquor Licenses Trade Here banner" />
      </section>

      <section className="ticker-shell" aria-label="Active Florida liquor license asking prices">
        <div className="ticker-line"><div className="ticker-track">{[...tickerOne,...tickerOne].map((listing,index)=><Link className="ticker-item" href={listingPageHref(listing)} key={`a-${listing.sourceRef}-${index}`}><b>{shortCounty(listing.county)}</b><span>{shortType(listing.type)}</span><strong>{money(listing.price,listing.priceLabel)}</strong><em>▲</em></Link>)}</div></div>
        <div className="ticker-line"><div className="ticker-track">{[...tickerTwo,...tickerTwo].map((listing,index)=><Link className="ticker-item" href={listingPageHref(listing)} key={`b-${listing.sourceRef}-${index}`}><b>{shortCounty(listing.county)}</b><span>{shortType(listing.type)}</span><strong>{money(listing.price,listing.priceLabel)}</strong><em>▲</em></Link>)}</div></div>
      </section>

      <div className="exchange-content">
        <section className="market-grid">
          <article className="panel"><div className="panel-heading"><div><strong>FLORIDA MARKET<br/>HEAT MAP</strong></div><small>ACTIVE 4COP &amp; 3PS<br/>LISTINGS BY COUNTY</small></div><div className="heatmap-wrap"><img src="/assets/fllm-exchange-heatmap.svg" alt="Florida liquor license market heat map" /></div><div className="panel-action"><Link className="gold-button" href="/?open=heat-map">Explore County Markets</Link></div></article>

          <article className="panel"><div className="board-title"><h1>FLLM EXCHANGE BOARD</h1><p>ACTIVE FLORIDA LIQUOR LICENSE ASKING PRICES</p></div><div className="board-table"><div className="board-head"><span>County</span><span>License Type</span><span>Asking Price</span><span>Status</span></div>{boardListings.map((listing)=><Link className="board-row" href={listingPageHref(listing)} key={listing.sourceRef}><span>{shortCounty(listing.county)}</span><span>{shortType(listing.type)}</span><strong>{money(listing.price,listing.priceLabel)}</strong><em>AVAILABLE</em></Link>)}</div><div className="panel-action" style={{textAlign:"center"}}><Link className="gold-button" href="/listings">View All Listings</Link></div></article>

          <aside><article className="panel"><div className="panel-heading"><div><strong>MARKET<br/>SNAPSHOT</strong></div><small>FLORIDA LIQUOR LICENSE<br/>MARKET</small></div><div className="snapshot-grid"><div><strong>{listings.length}</strong><span>Active Listings</span></div><div><strong>{countyCount}</strong><span>Counties</span></div><div><strong>{money(highest,"—")}</strong><span>Highest Ask</span></div><div><strong>{money(lowest,"—")}</strong><span>Lowest Ask</span></div></div></article><article className="panel newest-box"><div className="panel-heading"><div><strong>NEWEST TO<br/>MARKET</strong></div></div><div className="newest-list">{newest.map((listing)=><Link href={listingPageHref(listing)} key={listing.sourceRef}><b>NEW</b><span>{shortCounty(listing.county)} County</span><i>{shortType(listing.type)}</i><strong>{money(listing.price,listing.priceLabel)}</strong></Link>)}</div><div className="panel-action" style={{textAlign:"center"}}><Link className="gold-button" href="/listings">View Newest Listings</Link></div></article></aside>
        </section>

        <section className="news-section" aria-label="FLLM News market updates"><img src="/assets/market-report-studio.png" alt="FLLM Market News studio with Florida liquor license market update screens" /><div className="news-bar"><b>FLLM NEWS</b><span>EXPERTS. INSIGHTS. <em>A MORE TRANSPARENT MARKET.</em></span><Link className="gold-button" href="/florida-liquor-license-news">Watch Market Updates</Link></div></section>

        <section className="action-grid"><article className="action-card"><h2>FOR BUYERS</h2><p>Find available liquor licenses by county and license type.</p><Link className="gold-button" href="/listings">Browse Licenses</Link></article><article className="action-card"><h2>FOR SELLERS</h2><p>List your Florida liquor license and reach qualified buyers.</p><Link className="gold-button" href="/sell-your-license">List Your License</Link></article><article className="action-card"><h2>FOR BROKERS</h2><p>Market your client inventory while remaining the listing representative.</p><Link className="gold-button" href="/brokers/list-your-license">Broker Options</Link></article><article className="action-card"><h2>MARKET DATA</h2><p>Explore pricing, trends and county market information.</p><Link className="gold-button" href="/#market-data">View Market Data</Link></article></section>

        <section className="transparency-band"><h2>ADDING TRANSPARENCY TO THE FLORIDA LIQUOR LICENSE MARKET</h2><div className="transparency-values"><div><strong>MORE INFORMATION</strong><span>Listings, asking prices and county market context.</span></div><div><strong>MORE OPPORTUNITY</strong><span>A statewide view for buyers, sellers, brokers and lenders.</span></div><div><strong>A MORE TRANSPARENT MARKET</strong><span>Organized public information in a fragmented private market.</span></div></div></section>
