  ...
  return (
    <main className="exchange-page">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .approved-hero img{display:block;width:100%;height:auto;object-fit:contain;object-position:center}
            ...
            <div className="board-table">
              ...
              {boardListings.map((listing) => (
                <Link className="board-row" href={listingPageHref(listing)} key={listing.sourceRef}>
                  <span>{shortCounty(listing.county)}</span><span>{shortType(listing.type)}</span><strong>{money(listing.price, listing.priceLabel)}</strong><em>AVAILABLE</em>
                </Link>
              ))}
            </div>
