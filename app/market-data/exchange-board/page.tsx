  ...
  return (
    <main className="exchange-page">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .exchange-header{position:relative;z-index:20;background:#020d18;border-bottom:1px solid rgba(246,167,0,.55)}

        .exchange-hero{width:100%;background:#020b14;border-bottom:1px solid rgba(42,184,243,.35);line-height:0;overflow:hidden}
      ...
      <section className="exchange-hero" aria-label="FLLM Exchange trading floor">
        <img
          src="/assets/fllm-exchange-board-header-live.png?v=rebuild-1"
          width="975"
          height="332"
        ...
        <div className="ticker-line"><div className="ticker-track">
          {[...tickerOne, ...tickerOne].map((listing, index) => (
            <Link className="ticker-item" href={listingPageHref(listing)} key={`a-${listing.sourceRef}-${index}`}>
              <b>{countyName(listing.county)}</b>
              <span>{licenseType(listing.type)}</span>
            ...
            <div className="board-table">
              ...
              {boardListings.map((listing) => (
                <Link className="board-row" href={listingPageHref(listing)} key={listing.sourceRef}>
                  <span>{countyName(listing.county)}</span>
                  <span>{licenseType(listing.type)}</span>
                  <strong>{money(listing.price, listing.priceLabel)}</strong>
                  <em>AVAILABLE</em>
                </Link>
              ...
              <div className="newest-list">
                {newest.map((listing) => (
                  <Link href={listingPageHref(listing)} key={listing.sourceRef}>
                    <b>NEW</b>
                    <span>{countyName(listing.county)} County</span>
                    <i>{licenseType(listing.type)}</i>
                    <strong>{money(listing.price, listing.priceLabel)}</strong>
                  </Link>
                ))}
              </div>
