    ...
    <main className="results-page">
      ...
      <section className="results-content"><div className="page-shell">
        ...
        {filtered.length ? <div className="results-grid">{filtered.map((listing) => <article className="result-card" key={listing.sourceRef ?? `${listing.county}-${listing.price}`}>
          ...
          {listing.sourceRef ? <><small>{listing.note ?? countyDescriptions[listing.county] ?? "Price and availability subject to confirmation."}</small>{listing.sourceName ? <small>Third-party listing · Availability and price subject to confirmation</small> : null}<div className="result-actions"><Link className="btn btn-gold" href={`/contact?listing=${encodeURIComponent(`${listing.county} ${listing.type}`)}&ref=${listing.sourceRef}`}>Inquire</Link><Link className="btn offer-button" href={`/submit-offer?listing=${encodeURIComponent(`${listing.county} ${listing.type}`)}&ref=${listing.sourceRef}`}>Submit an Offer</Link></div></> : <div className="result-actions"><span className="sold-status">SOLD</span></div>}
