(function(){
  ...
  function positionResourcesMenus(){
    ...
    document.querySelectorAll('.contact-page > .fllm-official-contact-header .native-nav-resources-menu').forEach(function(menu){
      if(!(menu instanceof HTMLElement))return;
      var dropdown=menu.parentElement;
      var trigger=dropdown&&dropdown.querySelector('.native-nav-trigger');
      if(!(dropdown instanceof HTMLElement)||!(trigger instanceof HTMLElement))return;

      var triggerRect=trigger.getBoundingClientRect();
      var dropdownRect=dropdown.getBoundingClientRect();
      var computedWidth=parseFloat(window.getComputedStyle(menu).width)||860;
      var menuWidth=Math.min(computedWidth,window.innerWidth-24);
      var triggerCenter=triggerRect.left+triggerRect.width/2;
      var desiredViewportLeft=triggerCenter-menuWidth/2;
      var viewportLeft=Math.max(12,Math.min(desiredViewportLeft,window.innerWidth-menuWidth-12));
      var localLeft=viewportLeft-dropdownRect.left;
      var arrowLeft=triggerCenter-viewportLeft;
  ...
  function wireHeader(header){
    ...
    if(!globalInstalled){
      ...
      document.addEventListener('pointerover',function(event){
        ...
        if(target&&target.closest('.native-nav-dropdown')&&target.closest('.native-nav-dropdown').querySelector(':scope > .native-nav-resources-menu')){
