(() => {
  ...
  function positionOverlay(image, overlay, host) {
    ...
    const scaledLeft = imageRect.width * (isInsideVideoPopup ? 0.013 : 0.024);
    const popupPixelCorrection = isInsideVideoPopup ? -2 : 0;

    overlay.style.left = `${imageRect.left - hostRect.left + scaledLeft + popupPixelCorrection}px`;
